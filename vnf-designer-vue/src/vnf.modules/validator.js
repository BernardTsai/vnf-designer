...
var schema = {
    ...
    images: {
      ...
      items: {
        ...
        properties: {
          ...
          disk: { type: "string", description: "Minimum disk requirements" },

          size: { type: "string", description: "Size of the image" },

          checksum: { type: "string", description: "Checksum of the image" },

          url: { type: "string", description: "URL for the image" },
    ...
    networks: {
      ...
      items: {
        ...
        properties: {
          ...
          name: { type: "string", description: "Name of the virtual network" },
    ...
    components: {
      ...
      items: {
        ...
        properties: {
          ...
          user: { type: "string", description: "Name of the user account" },

          placement: {
            type: "string",
            enum: ["OTHER", "EXT", "INT", "MGMT", "ROUTER"],
          ...
          min: { type: "number", description: "Minimum size of the cluster" },

          size: { type: "number", description: "Default size of the cluster" },

          max: { type: "number", description: "Maximum size of the cluster" },

          userdata: { type: "string", description: "Initialisation script" },
            ...
            items: {
              ...
              properties: {
                ...
                network: {
                  type: "string",
                  description:
                    "The virtual network via which the service is exposed"
                },

                protocol: {
                  type: "string",
                  enum: ["tcp", "udp", "icmp", "sctp", "any", "none"],
                  description: "The service protocol"
                },

                range: { type: "string", minimum: 1, description: "Port range" }
              }
            }
          },
          ...
          dependencies: {
            description:
              "The dependencies to services provided by other componennts",
            type: "array",
            items: {
              ...
              properties: {
                ...
                service: {
                  type: "string",
                  description:
                    "The name of the service provided by the other component"
                },

                network: {
                  type: "string",
                  description:
                    "The virtual network via which the service is consumed"
                }
              }
            }
          }
  ...
  validate_schema(object) {
    ...
    if (!valid) {
      view.setFocus(verify.errors[0].dataPath);

      return (
        "Schema error:\nfield " +
        verify.errors[0].message +
        "\nLocation: " +
        verify.errors[0].dataPath
      );
    }
  ...
  validate_xref(object) {
    ...
    for (var index = 0; index < object.components.length; index++) {
      ...
      if (!flavors.includes(component.flavor)) {
        view.setFocus(".components[" + index + "].flavor");
      ...
      if (!images.includes(component.image)) {
        view.setFocus(".components[" + index + "].image");
      ...
      for (
        var subindex = 0;
        subindex < component.interfaces.length;
        subindex++
      ) {
        var interfac = component.interfaces[subindex];

        component_networks.push(interfac.network);

        if (!networks.includes(interfac.network)) {
          view.setFocus(
            ".components[" + index + "].interfaces[" + subindex + "].network"
          );
      ...
      for (var subindex = 0; subindex < component.services.length; subindex++) {
        ...
        if (!component_networks.includes(service.network)) {
          view.setFocus(
            ".components[" + index + "].services[" + subindex + "].network"
          );
      ...
      for (
        var subindex = 0;
        subindex < component.dependencies.length;
        subindex++
      ) {
        var dependency = component.dependencies[subindex];
        ...
        if (!component_networks.includes(dependency.network)) {
          view.setFocus(
            ".components[" + index + "].dependencies[" + subindex + "].network"
          );
        ...
        if (!services.includes(reference)) {
          view.setFocus(
            ".components[" + index + "].dependencies[" + subindex + "].service"
          );
